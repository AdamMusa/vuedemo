...
ARG CI="true" NIXPACKS_METADATA="node" NIXPACKS_NODE_VERSION NIXPACKS_SPA_OUTPUT_DIR="dist" NODE_ENV="production" NPM_CONFIG_PRODUCTION="false"
ENV CI=$CI NIXPACKS_METADATA=$NIXPACKS_METADATA NIXPACKS_NODE_VERSION=$NIXPACKS_NODE_VERSION NIXPACKS_SPA_OUTPUT_DIR=$NIXPACKS_SPA_OUTPUT_DIR NODE_ENV=$NODE_ENV NPM_CONFIG_PRODUCTION=$NPM_CONFIG_PRODUCTION
...
COPY . /app/.
RUN --mount=type=cache,id=IohCXITAdRo-/root/npm,target=/root/.npm npm i

# build phase
COPY . /app/.
RUN --mount=type=cache,id=IohCXITAdRo-node_modules/cache,target=/app/node_modules/.cache npm run build
